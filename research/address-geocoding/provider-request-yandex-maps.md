# Provider Request - Yandex Maps Geosuggest and Geocoder APIs

[Русская версия](provider-request-yandex-maps.ru.md)

This checklist is prepared for a vendor conversation with Yandex about Yandex Maps Geosuggest API and Yandex Maps Geocoder API. It must not be treated as provider answers until Yandex responds in writing or provides official documentation.

## Context

Atlas currently treats [`Yandex Maps Geosuggest API`](../../apis/yandex-maps-geosuggest-api/README.md) and [`Yandex Maps Geocoder API`](../../apis/yandex-maps-geocoder-api/README.md) as separate active reviewed products. Public Atlas blockers remain: production RPS, SLA, exact storage/display/SaaS rights, batch/offline restrictions, OpenAPI availability and independent quality benchmarks.
...
1. Please confirm the exact product boundary between Geosuggest API, Geocoder API, Search APIs, organization search, routing, matrix APIs and JavaScript map components.
...
4. Does Geocoder provide any address normalization or validation guarantees beyond geocoding precision metadata?
5. Which capabilities require separate licenses or contracts?

## Methods and Field Matrix

1. Please provide a complete method and parameter matrix for Geosuggest and Geocoder.
...
1. Please confirm production base URLs for Geosuggest and Geocoder.
2. Are API keys issued, restricted and billed separately for Geosuggest and Geocoder?
...
1. Are OpenAPI/Swagger specifications available for Geosuggest and Geocoder?
...
1. Please confirm API-specific pricing for Geosuggest and Geocoder by license type and request package.
...
6. How should a buyer price a workflow where each selected Geosuggest result triggers one or more Geocoder requests?

## Limits, Quotas and Production Suitability

1. What production requests-per-second limits apply to Geosuggest and Geocoder?
...
1. When must Geosuggest or Geocoder results be displayed on a Yandex map?
