...
Official Russian Treasury pages confirm the public role and identity of EIS / `zakupki.gov.ru`. Official agency open-data pages confirm that procurement-related CSV datasets exist. Existing Atlas research also preserves Seldon.Tenders as legacy-only because official API specification, auth, pricing, limits, SLA and data-rights evidence are missing.
...
Find current official EIS developer/service documentation, then compare it with Seldon.Tenders/Seldon.Win only if Seldon provides API-level evidence. Keep `datasets/procurement_tender_contracts.md` as legacy supporting research until then.
