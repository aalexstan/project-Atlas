...
FIAS/GAR is the official Russian registry route. The FNS open-data catalog verifies XML ZIP downloads, a structure ZIP, weekly updates and previous release links; Atlas has inspected the structure ZIP, the current data ZIP central directory, root-level dictionary XML files and one small regional directory sample. Official materials also mention SMEV and API services, but the current public method catalog, base URL, auth, quotas and SLA remain blockers, and other regions, national row counts and full/delta semantics are not verified.
...
- FIAS/GAR open-data XML ZIP route, structure XSD archive, current data ZIP central directory, root dictionary XML payload and sample regional directory `99/` payload are verified, but other regions, national row counts, full/delta semantics and API-service details remain incomplete in reviewed public pages.
