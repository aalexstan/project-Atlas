# Доказательства OpenCage Geocoding API
...
| Geocoding API provides worldwide geocoding based on open data via REST. | https://opencagedata.com/api | 2026-07-29 | verified | Provider positioning; benchmark still required. |
| API supports reverse geocoding and forward geocoding examples. | https://opencagedata.com/api | 2026-07-29 | verified | Product purpose. |
| Authentication uses API key passed as `key` query parameter. | https://opencagedata.com/api | 2026-07-29 | verified | No HTTP auth header required according to docs. |
...
| Supported response formats include JSON, GeoJSON, XML and Google-compatible JSON. | https://opencagedata.com/api | 2026-07-29 | verified | Google-compatible format described as convenience and may be discontinued. |
...
| API itself does not allow multiple locations per request. | https://opencagedata.com/api | 2026-07-29 | verified | Provider suggests spreadsheet upload or parallel requests for volume. |
...
Atlas credentialed request, benchmark, spreadsheet upload или live API test не проводились.
