...
| 2026-07-29 | Decompressed and parsed small root-level dictionary XML files from the current data ZIP. | Добавляет partial XML-payload evidence для reference/type dictionaries; regional payload, national row counts и full/delta semantics остаются blockers. |
| 2026-07-29 | Decompressed and parsed all XML files in sample regional directory `99/`. | Добавляет regional payload smoke-test evidence; national row counts, other regions и full/delta semantics остаются blockers. |
| 2026-07-29 | Decompressed and parsed all XML files in sample regional directory `87/` and validated CRC/size for the sampled entries. | Добавляет второй regional payload sample с integrity evidence; national row counts, remaining regions и full/delta semantics остаются blockers. |
| 2026-07-29 | Decompressed and parsed sparse sample regional directory `82/` and validated CRC/size for the sampled entries. | Добавляет edge-case sample с valid mostly-empty XML groups; national row counts, remaining regions и full/delta semantics остаются blockers. |
