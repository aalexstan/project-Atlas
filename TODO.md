...
- Для FIAS/GAR Data Integration после inspection official `structure-12032021.zip`, current data ZIP central directory и root dictionary XML payload проверить regional XML payload contents, national row counts, CRC validation against full payload, full/delta package model, API services method catalog, auth, quotas, costs, SMEV eligibility и legal-use rights.
