...
- Для FIAS/GAR Data Integration после inspection official `structure-12032021.zip`, current data ZIP central directory, root dictionary XML payload и sample regions `99/`/`87/` проверить remaining regional XML payload contents, national row counts, CRC validation beyond sampled regions, full/delta package model, API services method catalog, auth, quotas, costs, SMEV eligibility и legal-use rights.
