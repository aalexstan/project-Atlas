...
- Geoapify live-test технически выполнен 2026-08-25, но account-specific accepted Terms не сохранены как evidence: до `verified` и установки `live_tested_on/live_test_valid_until` закрыть этот blocker документом из кабинета или письменным подтверждением условий.
...
- После подтверждения условий free-tier выполнить предварительно зарегистрированные одиночные live-тесты LocationIQ и OpenCage из `research/*-geocoding-api/live-test-plan-2026-08-24*`; до получения законного доступа запросы не отправлять и ключи в репозиторий не добавлять.
- Для WeatherAPI.com получить официальное объяснение поведения `current_fields`/`fields` и gated страницы `/my/fields.aspx`; отдельно проверить fuzzy location matching как accuracy-риск, plan-specific limits, SLA, storage/redistribution и data rights; не повышать maturity без закрытия findings и независимой проверки.
- Для OpenWeather уточнить тариф, quota, SLA, storage/display/licensing и доступ к One Call API 3.0; не смешивать его с current/forecast API 2.5 и не считать HTTP 401 One Call доказательством невалидного key.
- Следующий live-test приоритет: переключиться на другую категорию, прежде всего address/geocoding, где live-test даст больше decision-value; не добавлять четвёртый weather API до отдельного решения о необходимости полноты weather comparison.
